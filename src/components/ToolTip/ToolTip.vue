<template>
  <div class="tooltip" ref="toolTip">
    <svg ref="svg" @click.stop="svgClick" class="svg" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div ref="tagWrap" class="tag-wrap" :class="{show: showTip}">
      ...
      <div class="tag-arrow" ref="tagArrow"></div>
    </div>
  </div>
</template>
...
onMounted(() => {
  // 默认悬浮窗向左位移半个悬浮窗宽度
  // 存在边界情况，位移后的悬浮窗可能超出屏幕
  // tagWrap.value.style.left = (-tagWrap.value.offsetWidth / 2) + 10 + 'px'
  tagWrap.value.style.top = -tagWrap.value.offsetHeight - 10 + 'px'
  const leftOffset = (tagWrap.value.offsetWidth / 2) - 10
  const exceedOverScreen = tagWrap.value.getBoundingClientRect().right - document.body.clientWidth - leftOffset
  // 如果定位之后依旧超出屏幕
  if (exceedOverScreen >= 0) {
    // 定位悬浮窗
    tagWrap.value.style.left = -(tagWrap.value.getBoundingClientRect().right - document.body.clientWidth) + 'px'
    // 在悬浮窗已经定位的基础上再来定位下角标
    tagArrow.value.style.left = svg.value.getBoundingClientRect().left - tagWrap.value.getBoundingClientRect().left + 10 +'px'
  } else {
    tagWrap.value.style.left = -leftOffset + 'px'
  }
  console.log(exceedOverScreen)
  // console.log(tagWrap.value.getBoundingClientRect().right, document.body.clientWidth, (-tagWrap.value.offsetWidth / 2))
  // tagWrap.value.style.left
})

const appClickListen = (event: Event) => {
  if (tagWrap.value && showTip.value && !tagWrap.value.contains(event.target)) {
    showTip.value = false
    document.getElementById('app')?.removeEventListener('click', appClickListen)
  }
}

const svgClick = () => {
  showTip.value = !showTip.value
  svgColor.value = showTip.value ? '#0089E5' : '#B8BABE'
  if (showTip.value) {
    document.getElementById('app')?.addEventListener('click', appClickListen)
  } else {
    document.getElementById('app')?.removeEventListener('click', appClickListen)
  }
}
